...
urlpatterns = [
    ...
    url(r'^api/v1/register/email/(?P<email>.+)/token/(?P<token>.+)/firstName/(?P<firstName>.+)/lastName/(?P<lastName>.+)/profileUrl/(?P<profileUrl>.+)$', api_controllers.create_new_user_v2, name='create_new_user_v2'),
